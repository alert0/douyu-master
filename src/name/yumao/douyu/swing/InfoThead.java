...
import name.yumao.douyu.util.SetSystemProperty;
...
public class InfoThead implements Runnable{
	...
	public void run() {
		...
		try {
		  ...
		  Config.cdn = SetSystemProperty.readValue("conf"+File.separator+"update.properties" , "cdn").trim() ;
		  
		  Config.rate = SetSystemProperty.readValue("conf"+File.separator+"update.properties" , "rate").trim() ;

			Config.PLAYLIST = Integer.parseInt(SetSystemProperty.readValue("conf"+File.separator+"update.properties" , "PLAYLIST").trim()) ;


			logger.info(Config.TIMEOUT + ":"+Config.CONNECTION_TIMEOUT +":"+ Config.SO_TIMEOUT +":" + Config.PLAYLIST);
