...
public class DouyuTCPDecoder extends CumulativeProtocolDecoder{
	private static Logger logger = Logger.getLogger(DouyuTCPDecoder.class);
...
//        	}
            ...
            if(contentLength > ioBuffer.remaining()){
            	//如果长度头大于最大body长度的话
            	//清空缓冲区内容获取下一步讯息
            	//logger.info("获取消息长度" +  contentLength  + "ioBuffer.remaining():" + ioBuffer.remaining() );
            	...
            	String msg = new String(subBytes(bodyBytes,8,bodyBytes.length-8),"UTF-8");
            //	logger.info(msg  );
            	out.write(msg);
            	//如果读取内容后还粘了包，就让父类再重读一次，进行下一次解析 
            	if(ioBuffer.remaining() > 0){
            		//logger.info("还粘了包" + ioBuffer.remaining()   );
                    return true; 
                } 
            }
		}
