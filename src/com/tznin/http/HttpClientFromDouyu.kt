...
class HttpClientFromDouyu {
    companion object {
        ...
        fun getHomeH5Enc(roomNum: Int): String {
            var response: Response? = null
            try {

                val apiurl = String.format("https://www.douyu.com/swf_api/homeH5Enc?rids=%s", roomNum)
...
//                    String msgStr = msgObj.toString();
                    val g = Gson()
                    val obj = g.fromJson(body, JsonObject::class.java)
                    if(!obj.get("error").asBoolean ) {
                         val data = g.fromJson(obj.get("data") , JsonObject::class.java)
                       // println(data.get("room"+ roomNum) )
                        return data.get("room"+ roomNum).asString
//                        for (set in data.entrySet()) {
//                          // println(set.key +"-"+ set.value)
//                        }

                    }
//                    println(obj.get("data"))
//                    for (set in obj.entrySet()) {
//                        println(set.key +"-"+ set.value)
//                    }
                }
...
//                //            get = new HttpGet(apiurl);
            ...
            //    var url  = sign + "&cdn=ws-h5&rate=0&ver=Douyu_219030605&iar=0&ive=0"

                val apiurl = String.format("https://www.douyu.com/lapi/live/getH5Play/%s",roomNum )
                ...
                var jsdata = JsInServer.jsrun(roomNum, uuid,time)
                println(jsdata )
                val list = jsdata.split("&")
                val v =  list[0].substringAfter("=")
                val sign =  list[3].substringAfter("=")
...
// 解密算法
//    object JsInServer {
//        internal var inv: Invocable? = null
//
//        fun jsrun(vid: Int, did: String, tt: String): String {
//            try {
//
//                if (inv != null)
...
//                val src = HttpClientFromDouyu.getHomeH5Enc(vid)
//                engine.eval(HttpClientFromDouyu.js)
//                engine.eval(src)
//
//
...
//
//                val d = inv!!.invokeFunction("ub98484234", vid, did, tt)
//
//                return d.toString()
//            } catch (e: Exception) {
